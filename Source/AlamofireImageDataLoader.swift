...
public class AlamofireImageDataLoader: ImageDataLoading {
    ...
    public func taskWith(request: ImageRequest, progress: ImageDataLoadingProgress, completion: ImageDataLoadingCompletion) -> NSURLSessionTask {
        let task = self.manager.request(request.URLRequest)
        task.response { _, response, data, error in
            completion(task: task.task, data: data, response: response, error: error)
            }.progress { (_, totalBytesReceived, totalBytesExpected) -> Void in
                progress(task: task.task, completed: totalBytesReceived, total: totalBytesExpected)
        }
        return task.task
    }
